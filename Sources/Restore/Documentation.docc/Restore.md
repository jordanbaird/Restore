...
Take snapshots of an object's state and later restore from them.
...
- Note: The `name` parameter in the ``Reference`` type's initializer must be the same as the name of the property, or the property will not be saved.
